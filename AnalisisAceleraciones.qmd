...
4.  **Detección automática de cambios**\
    Aplicación de técnicas de series temporales y análisis de señales para identificar transiciones de actividad:

    -   **Análisis espectral** (FFT, densidad espectral) para detectar variaciones periódicas.

    -   **Transformada wavelet** para localizar cambios de frecuencia en el tiempo.

    -   **Detección de puntos de cambio** (CPD, PELT) en media o varianza.

    -   **Métodos de clustering** (k‑means, DBSCAN) sobre vectores de características (magnitud, velocidad angular).

    -   **Modelos ocultos de Markov** para segmentar secuencias de movimiento.

    -   **Dynamic Time Warping** para comparar segmentos con patrones de referencia.
...
# Imprimir las tablas
kable(head(aceleracion_1, 10), caption = "Primeras 10 filas del dataset Aceleración 1")

kable(head(aceleracion_2, 10), caption = "Primeras 10 filas del dataset Aceleración 2")
```
...
1.  **Perí­odos de aceleración constante o nula** Secciones donde las lí­neas de aceleración permanecen cercanas a un valor constante durante periodos prolongados, sin fluctuaciones significativas.
    ...
    Graficar_un_intervalo(aceleracion_2, "AccZ", 10, 13)
    ```
...
resumen_aceleracion <- function(df, df_name = "Datos") {
  # Verificar que existan las columnas necesarias
  columnas_validas <- c("AccX", "AccY", "AccZ", "LinearAccelerometerSensor")
  if (!all(columnas_validas %in% colnames(df))) {
    stop("El dataframe debe contener las columnas AccX, AccY, AccZ y LinearAccelerometerSensor.")
  }

  # Calcular AccMag si no existe
  if (!"AccMag" %in% names(df)) {
    df <- df %>% mutate(AccMag = sqrt(AccX^2 + AccY^2 + AccZ^2))
  }

  # Seleccionar variables a resumir
  variables <- c("AccX", "AccY", "AccZ", "AccMag", "LinearAccelerometerSensor")
  ...
  # Imprimir encabezado
  cat("\n**Resumen estadístico -", df_name, "**\n\n")

  # Mostrar tabla formateada
  kable(resumen_df[, c("Variable", "Mínimo", "Q1", "Mediana", "Media", "Q3", "Máximo")])
}
...
resumen_aceleracion(aceleracion_1, "Aceleracion_1")
...
resumen_aceleracion(aceleracion_2, "Aceleracion_2")
```

-   En todos los estadísticos descriptivos de las variables se observa una amplia variabilidad en los valores mínimos y máximos de los componentes de aceleración, lo cual es indicativo de diferentes tipos de movimientos o interacciones físicas.
...
-   Del lado contrario, la variable LinearAccelerometerSensor representa la aceleración excluyendo el componente gravitacional, es decir, la aceleración real producida por el movimiento del dispositivo. Esta medida suele obtenerse mediante algoritmos de filtrado que separan la aceleración debida al movimiento de la producida por la gravedad.

    En los datos analizados, esta variable presenta valores promedio relativamente bajos en comparación con AccMag (alrededor de 0.3-0.4 m/s²), lo cual indica que gran parte del tiempo los dispositivos estuvieron en reposo o en movimientos suaves. Sin embargo, los valores máximos (hasta \~20 m/s² en algunos casos) sugieren la presencia de eventos de aceleración significativa, como sacudidas, impactos o movimientos rápidos.
